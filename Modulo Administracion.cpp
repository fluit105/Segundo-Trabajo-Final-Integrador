...
void rAsi() {
  ...
  if (users == NULL) {
    ...
    do {
      ...
      printf("-Empezar con letra minúscula\n");
      printf("-Poseer al menos dos letras mayúsculas\n");
      ...
      if (strlen(usr.usrID) > 10 || strlen(usr.usrID) < 6) {
        ...
        if (isupper(usr.usrID[0]) || isdigit(usr.usrID[0])) {
          ...
          system("cls");
          printf("\n\n\n**************************************************************************\n");
          printf("--> ERROR: El ID de usuario debe empezar con una letra minúscula\n");
          printf("**************************************************************************\n\n\n\n\n");
          system("pause");
        } else {
          int uppers = 0;
          ...
          if (err == 0) {
            for (int i = 0; i < strlen(usr.usrID); i++) {
              if (isalnum(usr.usrID[i]) || isupper(usr.usrID[i]) || usr.usrID[i] == '+' || usr.usrID[i] == '-' || usr.usrID[i] == '/' || usr.usrID[i] == '*' || usr.usrID[i] == '?' || usr.usrID[i] == '¿' || usr.usrID[i] == '!' || usr.usrID[i] == '¡') {
                ...
                system("cls");
                printf("\n\n\n*******************************************************************************\n");
                printf("--> ERROR: El ID de usuario solo admite los siguiente símbolos: +,-,/,*,?,Â¿,!,¡\n");
                printf("*******************************************************************************\n\n\n\n\n");
                system("pause");
                break;
              }
  ...
  if (vets == NULL) {
    printf ("ERROR: No se pudo abrir el archivo");
	  ...
	  fread(&vet, sizeof(veterin), 1, vets);
		...
		printf("*-*-*-*-*-*-           por número de atenciones            -*-*-*-*-*-*\n");
		printf("----------------------------------------------------------------------------\n\n");
